...
/container envs add list=sshgate-env key=SSH_HOSTS value="<real-host-1>:<port>,<real-host-2>"
/container envs add list=sshgate-env key=SSH_USER value="<real-user>"
/container envs add list=sshgate-env key=SSH_KEY value="id_rsa-VSCODE"

# Опционально — включить SOCKS-листенер с auth для теста
/container envs add list=sshgate-env key=SOCKS_PORT value="1080"
/container envs add list=sshgate-env key=SOCKS_USER value="testuser"
/container envs add list=sshgate-env key=SOCKS_PASS value="testpass"
...
/container envs remove [find list=sshgate-env key=SSH_HOSTS]
/container envs add list=sshgate-env key=SSH_HOSTS value="bad.example.com:22,<real-host-2>"
/container start number=<N>
```
